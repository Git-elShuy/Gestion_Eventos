...
export class CarrerasComponent implements AfterViewInit, OnInit {
        ...
        $('#miTabla').DataTable({
            ...
            //{ data: 'contacto', title: 'Contacto' },
            {
              ...
              render: function (data: any, type: any, row: any) {
                ...
                return `
                 ...
                 <i role="button" class="fa-solid fa-magnifying-glass editar-btn" data-id="${row.idcarreras}"></i> 
                 <i role="button" class="fa-solid fa-trash eliminar-btn" data-id="${row.idcarreras}" style="margin-left: 10px; color:red;"></i>`;
          ...
          initComplete: function () {
            ...
            $('#miTabla').on('click', '.eliminar-btn', function () {
              ...
              Swal.fire({
                title: `¿Estás seguro de que quieres eliminar ${nombreCarrera}?`,
                text: '',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                  const idcarrera = $(this).data('id');
                  that.eliminarCarrera(idcarrera);
                }
              });
  ...
  eliminarCarrera(idcarrera: number): void {
    this.cs.eliminarCarrera(idcarrera).subscribe({
      next: () => {
        Swal.fire({
  ...
  editarCarrera(idcarrera: number): void {
    this.router.navigate(['/dashboard/carreras/editar/', idcarrera]);
  }
